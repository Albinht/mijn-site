...
export default async function CopywritingPage() {
  ...
  const seoStepStyles = ['bg-[#1795FF] text-white', 'bg-[#FFE500] text-black']
  ...
  return (
    <ServicePageTemplate hero={heroData}>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="space-y-6">
            {copy.benefits.items.map((item) => (
              <div key={item.title} className="flex gap-4 p-6 bg-[#F5F5F5] border-2 border-black rounded-xl shadow-[4px_4px_0_0_#000]">
      ...
      <section className="py-16 md:py-24 px-6 bg-[#F5F5F5]">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-6">
            {copy.seoWriting.title.prefix}{' '}
            <span className="bg-[#1795FF] text-white px-2 rounded">{copy.seoWriting.title.highlight}</span>{' '}
            {copy.seoWriting.title.suffix}
          </h2>
          <p className="text-lg text-gray-700 mb-8">
            {copy.seoWriting.intro}
          </p>

          <div className="space-y-4">
            {copy.seoWriting.steps.map((step, index) => (
              <div key={step.text} className="flex gap-4">
                <div className={`flex-shrink-0 w-8 h-8 ${seoStepStyles[index % seoStepStyles.length]} border-2 border-black rounded-full flex items-center justify-center font-bold text-sm`}>
                  {step.number}
                </div>
