...
export default async function TechnicalSEOPage() {
  ...
  const niblahIcons = ['⚙️', '📊', '💻']
  const trackIcons = ['🚀', '🔧']
  const trackBorderStyles = ['border-[#1795FF]', 'border-[#FFE500]']
  const pillarStyles = ['bg-[#1795FF] text-white', 'bg-[#FFE500] text-black']
  ...
  return (
    <ServicePageTemplate hero={heroData}>
      ...
      <section className="py-16 md:py-24 px-6 bg-[#F5F5F5]">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4 text-center">
            {copy.tracks.title.prefix}{' '}
            <span className="bg-[#FFE500] px-2 rounded">{copy.tracks.title.highlight}</span>{' '}
            {copy.tracks.title.suffix}
          </h2>
          <p className="text-lg text-gray-600 text-center mb-12 max-w-3xl mx-auto">
            ...
            {copy.tracks.items.map((item, index) => (
              <div key={item.title} className="bg-white border-2 border-black rounded-xl p-8 shadow-[6px_6px_0_0_#000]">
                <div className="flex items-center gap-3 mb-6">
                  <div className={`w-12 h-12 ${index % 2 === 0 ? 'bg-[#1795FF] text-white' : 'bg-[#FFE500] text-black'} border-2 border-black rounded-full flex items-center justify-center text-2xl`}>
                    {trackIcons[index]}
                  </div>
                ...
                <div className="space-y-4">
                  {item.items.map((detail) => (
                    <div key={detail.title} className={`border-l-4 ${trackBorderStyles[index]} pl-4`}>
                      <p className="font-semibold text-gray-900 mb-1">{detail.title}</p>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="space-y-6">
            {copy.pillars.items.map((pillar, index) => (
              <div key={pillar.title} className="bg-[#F5F5F5] border-2 border-black rounded-xl p-8 shadow-[4px_4px_0_0_#000]">
                <div className="flex items-start gap-6">
                  <div className={`flex-shrink-0 w-16 h-16 ${pillarStyles[index % pillarStyles.length]} border-2 border-black rounded-xl flex items-center justify-center font-bold text-2xl`}>
                    {pillar.number}
                  </div>
                  <div>
