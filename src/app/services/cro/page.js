...
import { getCROPageCopy } from '@/i18n/services/cro-page'

export async function generateMetadata() {
  const locale = await getServerLocale()
  const copy = getCROPageCopy(locale)
  ...
  return (
    <ServicePageTemplate hero={heroData}>
          ...
          <div className="space-y-4 text-lg text-gray-700 leading-relaxed">
            ...
            <div className="space-y-3 ml-6">
              {copy.why.benefits.map((benefit) => (
                <p key={benefit} className="text-gray-700">
                  {benefit}
                </p>
              ))}
            </div>
      ...
      <section className="py-16 md:py-24 px-6 bg-white">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4 text-center">
            {copy.expertise.title.prefix ? `${copy.expertise.title.prefix} ` : ''}
            <span className="bg-[#1795FF] text-white px-2 rounded">{copy.expertise.title.highlight}</span>
            {copy.expertise.title.suffix ? ` ${copy.expertise.title.suffix}` : ''}
          </h2>
          <p className="text-lg text-gray-600 text-center mb-12 max-w-3xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {copy.expertise.items.map((item, index) => (
              <div key={item.title} className="bg-[#F5F5F5] border-2 border-black rounded-xl p-8 shadow-[6px_6px_0_0_#000]">
                ...
                <p className="text-gray-600 mb-4">{item.paragraphs[0]}</p>
                <p className="text-gray-600">{item.paragraphs[1]}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-16 md:py-24 px-6 bg-[#F5F5F5]">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4 text-center">
            {copy.approach.title.prefix ? `${copy.approach.title.prefix} ` : ''}
            <span className="bg-[#FFE500] px-2 rounded">{copy.approach.title.highlight}</span>
            {copy.approach.title.suffix ? ` ${copy.approach.title.suffix}` : ''}
          </h2>
          <p className="text-lg text-gray-600 text-center mb-12 max-w-3xl mx-auto">
          ...
          <div className="space-y-6">
            {copy.approach.steps.map((step, index) => (
              <div key={step.number} className="bg-white border-2 border-black rounded-xl p-8 shadow-[6px_6px_0_0_#000]">
                <div className="flex items-start gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-gray-700 mb-4">{step.paragraphs[0]}</p>
                    <p className="text-gray-700">{step.paragraphs[1]}</p>
                  </div>
                </div>
              </div>
            ))}
