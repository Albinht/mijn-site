...
import { getShopifyMarketingCopy } from '@/i18n/shopify/marketing-page'

export async function generateMetadata() {
  const locale = await getServerLocale()
  const copy = getShopifyMarketingCopy(locale)
  return copy.metadata
}

export default async function ShopifyMarketingPage() {
  const locale = await getServerLocale()
  const copy = getShopifyMarketingCopy(locale)
    ...
    <ServicePageTemplate hero={heroData}>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
              {copy.sections.activities.title.prefix}{' '}
              <span className="bg-[#1795FF] text-white px-2 rounded">{copy.sections.activities.title.highlight}</span>
              {copy.sections.activities.title.suffix ? ` ${copy.sections.activities.title.suffix}` : ''}
            </h2>
            <p className="text-lg text-gray-600 italic max-w-3xl mx-auto">{copy.sections.activities.quote}</p>
          </div>

          <div className="grid md:grid-cols-3 gap-8">
            {copy.sections.activities.cards.map((card) => (
      ...
      <section className="py-16 md:py-24 px-6 bg-white">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
              {copy.sections.channels.title.prefix}{' '}
              <span className="bg-[#1795FF] text-white px-2 rounded">{copy.sections.channels.title.highlight}</span>
              {copy.sections.channels.title.suffix ? ` ${copy.sections.channels.title.suffix}` : ''}
            </h2>
            <p className="text-lg text-gray-600 max-w-3xl mx-auto">{copy.sections.channels.subtitle}</p>
          </div>

          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {copy.sections.channels.cards.map((card) => (
      ...
      <section className="py-16 md:py-24 px-6 bg-gray-50">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-6">
            {copy.sections.partner.title.prefix}{' '}
            <span className="bg-[#1795FF] text-white px-2 rounded">{copy.sections.partner.title.highlight}</span>
            {copy.sections.partner.title.suffix ? ` ${copy.sections.partner.title.suffix}` : ''}
          </h2>
          {copy.sections.partner.paragraphs.map((paragraph) => (
