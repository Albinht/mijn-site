...
import { getShopifyThemaCopy } from '@/content/shopify/thema-page'

export async function generateMetadata() {
  const copy = getShopifyThemaCopy()
  return copy.metadata
}

export default async function ShopifyThemaPage() {
  const copy = getShopifyThemaCopy()
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
              ...
              {copy.sections.features.title.suffix ? ` ${copy.sections.features.title.suffix}` : ''}
            </h2>
          </div>

          <div className="grid md:grid-cols-2 gap-6">
            {copy.sections.features.items.map((item) => (
