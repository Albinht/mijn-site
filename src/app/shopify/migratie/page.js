...
import { getShopifyMigratieCopy } from '@/content/shopify/migratie-page'

export async function generateMetadata() {
  const copy = getShopifyMigratieCopy()
  return copy.metadata
}

export default async function ShopifyMigratiePage() {
  const copy = getShopifyMigratieCopy()
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
              ...
              {copy.sections.process.title.suffix ? ` ${copy.sections.process.title.suffix}` : ''}
            </h2>
            <p className="text-lg text-gray-600 max-w-3xl mx-auto">{copy.sections.process.subtitle}</p>
          </div>

          <div className="grid md:grid-cols-2 gap-6">
            {copy.sections.process.cards.map((card) => (
