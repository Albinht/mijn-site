...
import { getWordPressPartnerCopy } from '@/content/wordpress/partner-page'

export async function generateMetadata() {
  const copy = getWordPressPartnerCopy()
  return copy.metadata
}

export default async function WordPressPartnerPage() {
  const copy = getWordPressPartnerCopy()
    ...
    <ServicePageTemplate hero={heroData}>
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-6">
                {copy.sections.expert.title.prefix}{' '}
                <span className="bg-[#1795FF] text-white px-2 rounded">{copy.sections.expert.title.highlight}</span>
                {copy.sections.expert.title.suffix ? ` ${copy.sections.expert.title.suffix}` : ''}
              </h2>
              <div className="space-y-4 text-gray-600">
                {copy.sections.expert.paragraphs.map((paragraph, index) => (
                  <p key={paragraph} className={index === copy.sections.expert.paragraphs.length - 1 ? 'font-semibold text-gray-900' : ''}>
                    {paragraph}
                  </p>
                ))}
              </div>
            ...
            <div className="relative">
              <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl p-8 border-2 border-black shadow-[8px_8px_0_0_#000]">
                <h3 className="text-xl font-bold text-gray-900 mb-6">{copy.sections.expert.listHeading}</h3>
                <div className="space-y-4">
                  {copy.sections.expert.listItems.map((item) => (
      ...
      <section className="py-16 md:py-24 px-6 bg-white">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-gray-600 max-w-3xl mx-auto">{copy.sections.services.subtitle}</p>
          </div>
      ...
      <section className="py-16 md:py-24 px-6 bg-gray-50">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-gray-600 max-w-3xl mx-auto">{copy.sections.process.subtitle}</p>
          </div>

          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {copy.sections.process.steps.map((step) => (
              <div key={step.title} className="bg-white border-2 border-black rounded-xl p-6 shadow-[4px_4px_0_0_#000]">
      ...
      <section className="py-16 md:py-24 px-6 bg-white">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <Link
            href={copy.sections.cta.buttonLink}
