...
import { getWordPressWebsiteLatenMakenCopy } from '@/i18n/wordpress/website-laten-maken-page'

export async function generateMetadata() {
  const locale = await getServerLocale()
  const copy = getWordPressWebsiteLatenMakenCopy(locale)
  return copy.metadata
}

export default async function WordPressWebsitePage() {
  const locale = await getServerLocale()
  const copy = getWordPressWebsiteLatenMakenCopy(locale)
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-6">
                ...
                {copy.sections.power.title.suffix}
              </h2>
              <div className="space-y-4 text-gray-600">
                {copy.sections.power.paragraphs.map((paragraph, index) => (
                  <p key={paragraph} className={index === copy.sections.power.paragraphs.length - 1 ? 'font-semibold text-gray-900' : ''}>
                    {paragraph}
                  </p>
                ))}
              </div>
            ...
            <div className="relative">
              <div className="bg-white rounded-2xl p-8 border-2 border-black shadow-[8px_8px_0_0_#000]">
                <div className="space-y-6">
                  {copy.sections.power.bullets.map((bullet) => (
                    <div key={bullet} className="flex items-start gap-4">
                      ...
                      <p className="text-gray-900 font-medium">{bullet}</p>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {copy.sections.benefits.items.map((item) => (
              <div
                key={item}
                className="flex items-start gap-4 bg-white p-6 rounded-xl border-2 border-black shadow-[4px_4px_0_0_#000]"
              >
                <div className="w-8 h-8 flex-shrink-0 mt-1">
                  <Image src="/wp dev.png" alt={listIconAlt} width={32} height={32} className="w-full h-full object-contain" />
                </div>
