...
import { getWordPressThemaCopy } from '@/i18n/wordpress/thema-page'

export async function generateMetadata() {
  const locale = await getServerLocale()
  const copy = getWordPressThemaCopy(locale)
  return copy.metadata
}

export default async function WordPressThemaPage() {
  const locale = await getServerLocale()
  const copy = getWordPressThemaCopy(locale)
    ...
    <ServicePageTemplate hero={heroData}>
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            ...
            <div className="relative">
              <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl p-8 border-2 border-black shadow-[8px_8px_0_0_#000]">
                <h3 className="text-xl font-bold text-gray-900 mb-6">{copy.sections.why.listHeading}</h3>
                <div className="space-y-4">
                  {copy.sections.why.listItems.map((item) => (
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
              ...
              {copy.sections.benefits.title.suffix}
            </h2>
            <p className="text-lg text-gray-600 italic max-w-3xl mx-auto">{copy.sections.benefits.quote}</p>
          </div>
      ...
      <section className="py-16 md:py-24 px-6 bg-white">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
              {copy.sections.features.title.prefix}{' '}
              <span className="bg-[#1795FF] text-white px-2 rounded">{copy.sections.features.title.highlight}</span>{' '}
              {copy.sections.features.title.suffix}
            </h2>
          </div>

          <div className="grid md:grid-cols-2 gap-6">
            {copy.sections.features.cards.map((card) => (
      ...
      <section className="py-16 md:py-24 px-6 bg-gray-50">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-6">
            {copy.sections.setup.title.prefix}{' '}
            <span className="bg-[#1795FF] text-white px-2 rounded">{copy.sections.setup.title.highlight}</span>{' '}
            {copy.sections.setup.title.suffix}
          </h2>
          {copy.sections.setup.paragraphs.map((paragraph) => (
            <p key={paragraph} className="text-lg text-gray-600 mb-8 leading-relaxed">
              {paragraph}
            </p>
          ))}
          <Link
            href={buildLocalizedPath(copy.sections.setup.buttonLink, locale)}
          ...
          >
            {copy.sections.setup.buttonText} <span>→</span>
          </Link>
        </div>
      </section>
